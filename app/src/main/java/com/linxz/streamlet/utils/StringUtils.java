...
3、将奇数位总和加上偶数位总和，结果应该可以被10整除。       
    ...
    /**365C端项目获取参数加密签名sign*/
    public static String getMJH_C_Sign(Map<String, String> paramsHsp){
        String ascllParams=formatUrlMap(paramsHsp,true,false);
        return Md5("secret"+ascllParams+"secret");
    }
        ...
        try {
            ...
            for (String key : keys) {
                ...
                if (StringUtils.areNotEmpty(key, value+"") && !"Sign".equals(key)
                        && !"Method".equals(key) && !"SignKeyToken".equals(key)) {
                    query.append(key).append(value);
                }
            }
            LogUtil.d("","       加密字符串："+query.toString()+"        ");
            // 第三步：使用MD5/HMAC加密
    ...
    public static String byte2hex(byte[] bytes) {
        ...
        LogUtil.d("","       "+sign.toString()+"        ");
        LogUtil.d("","=====================开始结束==================");
        return sign.toString();
    }

    public static  boolean areNotEmpty(String key, String val){
